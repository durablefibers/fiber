...
| Lease lost | `Welcome` carries `lease_secs` (300). If the agent has not reconnected within that minus three heartbeats (**270 s**), it stops its in-flight steps without reporting them and drops their buffered lines — the server has, or is about to, reclaim the lease. Server side, the reclaim loop requeues an expired lease with the same budget as before: it counts against `retries` with one extra try, and past `retries + 1` lost leases the step fails with `lease lost after N attempts`. A completion that arrives after the reclaim is ignored as stale |
...
| Stale | No heartbeat for `FIBER_AGENT_STALE_SECS` (default **45**) → marked offline and its socket, if a replica still holds one, is closed so it reconnects. Its leases are not touched; they expire on their own if it never comes back |

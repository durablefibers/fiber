...
- **Reclaim is bounded.** An expired lease or a disconnect requeues a step only while its lost attempt is within `retries`; after that it fails and the run propagates, so a step that kills its agent cannot be leased forever.

...
| Lease lost | `Welcome` carries `lease_secs` (300, clamped to a day). If no session has written a heartbeat within that minus three heartbeats (**270 s**, counted from the last heartbeat the socket accepted before the outage — a reconnect that fails, or connects and drops before a heartbeat, does not move it), the agent stops its in-flight steps without reporting them and drops everything it still holds about them, including a completion for a step that finished while disconnected — the server has, or is about to, reclaim the lease. A new offer for the same step likewise purges anything queued from the earlier attempt. Server side, the reclaim loop requeues an expired lease with the same budget as before: it counts against `retries` with one extra try, and past `retries + 1` lost leases the step fails with `lease lost after N attempts`. A completion that arrives after the reclaim is ignored as stale |

...
- **Redaction now covers the encodings a step produces by accident.** Masking matched a
  ...
  trace, or a value serialized into a JSON body wrote the secret in clear into
  `log_lines`, which any project `reader` can fetch. Each secret of at least 8 characters
  is now registered as its base64 (standard, URL-safe, and the standard encoding with the
  trailing newline `base64 <<<` adds), percent-encoded and JSON-escaped forms as well —
  each line of a multi-line secret included. The remaining limits are written down in
  [agents](docs/agents.md#what-masking-covers).
- **An agent can download only the artifacts its step depends on.** The restore list an
  offer carries was limited to the step's transitive `needs` closure, but the download
  route checked only that the agent held *some* running step of the run: an agent holding
  one branch could fetch a parallel branch's artifacts by id. Both now use the same
  closure. Exploiting it needed the artifact's random UUID, so this is defence in depth.
- **A presigned artifact upload is bound to its size, and a rejected one no longer leaks
  an object.** The PUT URL had no size condition — a licence to write any number of bytes
  into the bucket for ten minutes — and a `complete` the API refused (wrong size, over a
...
- **The durable `http_request` task sends to the address it checked.** It resolved the
  ...
  request. The checked addresses are now pinned onto the client.
- **Caps on two authenticated ways to exhaust the instance.** `interval_task` chains stop
  rather than extending themselves once a project has `FIBER_INTERVAL_MAX_PER_PROJECT`
  (10) live ones, and a step may store `FIBER_MAX_ARTIFACTS_PER_STEP` (50) artifacts
  totalling `FIBER_MAX_ARTIFACT_BYTES_PER_STEP` (512 MiB). Re-uploading the same name does
  not count twice, so an at-least-once retry is unaffected.
...
- **A directory under `artifacts:` fails the step** instead of being skipped with a note
  ...
  dist.tgz dist`) and declare the archive.
- **Retention catches up.** It deleted at most `FIBER_RETENTION_BATCH` (100) runs an hour
  — a ceiling of 2 400 a day a busy project never reaches — and one batch's cascade could
  put millions of `log_lines` rows in a single implicit transaction. A tick now takes up
  to 20 full batches, and each batch's log lines go in 50 000-row statements before the
  runs, with a log line naming the knob to raise if the backlog is still growing.
- **`/metrics` costs one round of queries per 10 seconds, not per scrape**, and both
  latency histograms cover the last 24 hours rather than every attempt inside the
  retention window (two sequential scans of `step_attempts` every scrape).
...
- **`fiber run --wait/--follow` no longer reports a proxy hiccup as a failed build.** One
  ...
  entirely, since it was only checked between polls. Polls now have a 30-second request
  timeout and are retried five times with backoff, bounded by the deadline.
- An artifact upload the API refuses now shows the server's reason in the step log rather
  than only `HTTP 400`.
- `scripts/smoke_s3_presign.py` derives the expected bucket from `FIBER_S3_BUCKET` instead
  of hardcoding `fiber-artifacts`, so it can be run against a throwaway bucket.

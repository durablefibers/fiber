...
function SidebarProvider({
  ...
  // Restore cookie after SSR hydration (document is unavailable on the server).
  React.useEffect(() => {
    if (openProp !== undefined) return
    const stored = readSidebarOpenCookie(defaultOpen)
    _setOpen(stored)
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // Persist collapse/expand so refresh restores icon-only vs full sidebar.

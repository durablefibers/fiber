...
- **Offers are all or nothing.** The lease is taken first and the offer built second, from the snapshot, the project's secrets and the run's artifacts; if any of those cannot be read, the lease is backed out (the step returns to the queue in its original position, no attempt spent) and nothing is sent. An agent is offered steps until its slots are full, oldest-queued first, and its slot count comes from `step_runs` rather than from what a replica remembers delivering.

...
JSON bodies — and an upload for an attempt the row has moved past is `401`. Without it
...
has no business being there. An agent token leases steps and receives project secrets. The
API logs a warning when one arrives that way. `/ws/runs/{id}` takes the subprotocol only: a
session token is the whole user's authority for two weeks, and nothing needed it in the URL.

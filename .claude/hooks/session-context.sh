...
ui="down"; curl -sf -m 1 -o /dev/null http://127.0.0.1:3100 2>/dev/null && ui="up (:3100)"
...
echo "Fiber stack: api=$api ui=$ui postgres=$pg redis=$redis fiber-agent processes=$agents"

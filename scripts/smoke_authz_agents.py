...
def main() -> int:
    ...
    code, add = req(
        ...
        token=admin,
        body={"username": "smoke_reader", "role": "reader", "password": "fiber-reader"},
    )
    ...
    check(
        "list members",
        code == 200 and any(m.get("username") == "smoke_reader" for m in members),
        members,
    )

    code, rlogin = req(
        "POST", "/api/auth/login", body={"username": "smoke_reader", "password": "fiber-reader"}
    )
    ...
    code, created = req(
        ...
        "/api/agents",
        token=admin,
        body={"name": "smoke-agent", "labels": ["os=linux", "smoke=true"], "concurrency": 2},
    )
    ...
    code, updated = req(
        ...
        f"/api/agents/{aid}",
        token=admin,
        body={"concurrency": 3, "labels": ["os=linux", "smoke=rotated"]},
    )
    check(
        "update agent",
    ...
    check(
        "list agents includes the smoke agent",
    ...
    reader_id = next(m["user_id"] for m in members if m.get("username") == "smoke_reader")
    # refresh members after add may have different shape
    code, members2 = req("GET", f"/api/projects/{pid}/members", token=admin)
    reader_id = next(m["user_id"] for m in members2 if m.get("username") == "smoke_reader")
    ...
    pw_user, pw_first, pw_second = "smoke_pw", "initial-pass-1", "changed-pass-2"
